...
const mockBulletins = [
  ...
  { 
    ...
    date: '15/01/2024',
    ranking: '15/32',
    appreciation: 'Premiers résultats encourageants.',
    subjects: [
      { name: 'Mathématiques', average: 13.5, coefficient: 4 },
      { name: 'Français', average: 12.5, coefficient: 4 },
      { name: 'Physique', average: 14.0, coefficient: 3 },
      { name: 'Anglais', average: 14.5, coefficient: 2 },
      { name: 'Histoire', average: 13.0, coefficient: 2 },
      { name: 'SVT', average: 13.0, coefficient: 3 },
    ]
  },
];
...
export default function StudentBulletins() {
  ...
  const [selectedBulletin, setSelectedBulletin] = useState<typeof mockBulletins[0] | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  // Filter bulletins
  const filteredBulletins = mockBulletins.filter(bulletin => {
  ...
  const handleViewBulletin = (bulletin: typeof mockBulletins[0]) => {
    setSelectedBulletin(bulletin);
    setIsModalOpen(true);
  };

  const handleDownloadPDF = (bulletin: typeof mockBulletins[0]) => {
    console.log('Download PDF:', bulletin.id);
    alert(`Téléchargement du bulletin ${bulletin.semester} ${bulletin.academicYear}...`);
  };

  const availableCount = mockBulletins.filter(b => b.status === 'available').length;

  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold tracking-tight">Mes Bulletins</h1>
        <p className="text-sm text-muted-foreground">
          Téléchargez vos bulletins de notes ({availableCount} disponibles)
        </p>
      </div>
          ...
          {selectedBulletin && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Année académique {selectedBulletin.academicYear}
                </DialogDescription>
              </DialogHeader>
